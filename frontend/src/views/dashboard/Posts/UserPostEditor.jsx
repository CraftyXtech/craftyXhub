...
export default function UserPostEditor() {
  ...
  }, [settingsOpen, handleAutoSave]);

  const handleGenerateExcerpt = useCallback(async () => {
    if (!editorData?.blocks || isEditorEmpty(editorData)) {
      setError('Write the article first, then generate the excerpt from the full draft.');
      return;
    }
    ...
    try {
      ...
      setManualExcerpt(result.excerpt || '');
      setIsDirty(true);
    } catch (err) {
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      formData.append('content', editorJsToHtml(editorData) || '');
  ...
  useEffect(() => {
    if (!settingsOpen || !isDirty) return;
    if (!editorData?.blocks || isEditorEmpty(editorData)) return;
    ...
    autoSaveTimeoutRef.current = setTimeout(() => {
      handleAutoSave({
        blocks: editorData,
        html: editorJsToHtml(editorData),
      });
    }, 4000);
