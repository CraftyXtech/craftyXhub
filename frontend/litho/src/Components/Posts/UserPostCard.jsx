...
const UserPostCard = ({ post, onDelete, onPublish, className, ...props }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            console.log(`${isDraft ? 'Draft' : 'Post'} deleted successfully:`, post.uuid)
        } catch (error) {
            console.error(`Error deleting ${isDraft ? 'draft' : 'post'}:`, error)
            alert(`Failed to delete ${isDraft ? 'draft' : 'post'}. ${error.message || 'Please try again.'}`)
        } finally {
            setIsDeleting(false)
    ...
    const handlePublish = async () => {
        try {
            ...
            // Call parent callback to refresh the lists
            if (onPublish) {
                onPublish(post.uuid, result)
            }
            
            console.log('Draft published successfully:', post.uuid, result)
        } catch (error) {
            console.error('Error publishing draft:', error)
            alert(`Failed to publish draft. ${error.message || 'Please try again.'}`)
        } finally {
    ...
    return (
        <m.div 
            ...
            {...props}
        >
            {/* Responsive Layout: Vertical for mobile, Horizontal for larger screens */}
            <div className="flex flex-col xl:flex-row">
                {/* Featured Image Section - Top/Left */}
                <div className="xl:w-1/3 flex-shrink-0">
                    {featuredImageUrl ? (
                        <div className="relative h-48 xl:h-full bg-gray-100">
                            <img
                        ...
                        <div className="h-48 xl:h-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                            <div className="text-center">
                                <i className="feather-image text-4xl text-gray-400 mb-2"></i>
                ...
                <div className="xl:w-2/3 p-6 flex flex-col justify-between">
                    <div>
                        {/* Title */}
                        ...
                        {/* Metadata */}
                        <div className="flex items-center justify-between text-xs text-spanishgray mb-4">
                        ...
                        <div className="flex flex-wrap gap-2 mt-auto">
                            ...
                            {isDraft && onPublish && (
                                <Buttons
                                    ...
                                    className={`font-medium font-serif uppercase text-xs flex-1 min-w-[120px] ${isPublishing ? 'loading' : ''}`}
                            ...
                            <button
                                onClick={handleDelete}
                                className="px-3 py-2 text-xs font-medium text-red-600 hover:text-red-800 hover:bg-red-50 rounded-md transition-colors flex items-center justify-center"
