...
export const getImageUrl = (imagePath, folder) => {
  ...
  return `${apiBase}/v1/uploads/images/${filename}?folder=${folder || "posts"}`;
};
...
export const getPostImage = async (filename) => {
  try {
    const response = await axiosInstance.get(
      `/uploads/images/${filename}?folder=posts`
    );
    return response.data;
  } catch (error) {
    throw error;
...
export const getRelatedPosts = async (postUuid, params = {}) => {
  try {
    const response = await axiosInstance.get(`/posts/${postUuid}/related`, {
      params,
    });
    return response.data;
  } catch (error) {
...
export const createProfile = async (profileData) => {
  try {
    ...
    if (profileData.twitter_handle)
      formData.append("twitter_handle", profileData.twitter_handle);
    if (profileData.github_handle)
      formData.append("github_handle", profileData.github_handle);
    if (profileData.linkedin_handle)
      formData.append("linkedin_handle", profileData.linkedin_handle);
...
export const updateProfile = async (userUuid, profileData) => {
  try {
    ...
    if (profileData.twitter_handle !== undefined)
      formData.append("twitter_handle", profileData.twitter_handle || "");
    if (profileData.github_handle !== undefined)
      formData.append("github_handle", profileData.github_handle || "");
    if (profileData.linkedin_handle !== undefined)
      formData.append("linkedin_handle", profileData.linkedin_handle || "");
...
export const getUserBookmarks = async (params = {}) => {
  try {
    const response = await axiosPrivate.get("/posts/users/me/bookmarks", {
      params,
    });
    return response.data;
  } catch (error) {
